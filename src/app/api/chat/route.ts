...
// ─── System Prompt ─────────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `
You are Zara — a smart, warm sales advisor for OrganoCity, a premium product store from Pakistan.
...
KEY PRODUCTS:
- Products → culinary, home, bath, wellness, and gifting use
- Shilajit products → creative work, development, apple ecosystem
...
- Phone: +92 317 1707418
- Email: organocitypk@gmail.com
...
function isProductQuery(text: string) {
  return /pink salt|black salt|salt lamp|shilajit|herbal|honey|dry fruits|wellness|product|show|list|what do you|recommend|available|price|buy|order|purchase/i.test(
    text
  );
}
  ...
  try {
    ...
    if (showProducts) {
      ...
      return NextResponse.json({
        ...
        replyHtml: buildProductCards(products, "Here's what we have for you 💻 — latest products, Shilajit products & accessories:"),
        recommendations: products.map((p) => ({
          id: p.id, variantId: p.id, title: p.title,
          price: String(p.price), image: p.featuredImage,
        })),
...
// ── No API key fallback ──────────────────────────────────────────────────
    if (!process.env.OPENAI_API_KEY) {
      return NextResponse.json({
        reply: "Ask me about products, Shilajit products, or accessories!",
        replyHtml: `
          <p style="font-size:14px; color:${C.muted}; line-height:1.65; margin:0 0 8px;">I'm here to help you find the perfect product! 💻</p>
          <p style="font-size:14px; color:${C.text}; margin:0;">Ask me about ${hl("Products")}, ${hl("Shilajit products")}, or our ${hl("Accessories")} — or say <span style="color:${C.gold}; font-weight:600;">"show me products"</span> to browse!</p>`,
        draftOrder,
      });
    }
          ...
          ? [
              {
                ...
                content: `Current product catalog:\n${productContext}\n\nProduct cards are rendered separately — you ONLY return the conversational HTML reply. Always highlight product names in <strong style="color:#f6a45d;"> and prices in <span style="color:#C6A24A; font-weight:700;">.`,
              },
            ]
          : []),
