...
export function CertificateShowcase({ certificates }: { certificates: PublicCertificate[] }) {
  ...
  return (
    <main className="bg-gray-50 text-gray-900">
      <section className="relative overflow-hidden border-b border-[#C6A24A]/35 bg-[#fcf5e8] px-5 py-20 sm:py-24">
        <div className="absolute right-0 top-0 -mr-32 -mt-32 h-80 w-80 rounded-full bg-[#C6A24A]/25 blur-3xl" /><div className="absolute bottom-0 left-0 -mb-32 -ml-32 h-80 w-80 rounded-full bg-[#f6a45d]/20 blur-3xl" />
        <div className="relative mx-auto max-w-5xl text-center"><span className="inline-flex items-center gap-2 rounded-full border border-[#C6A24A]/50 bg-[#C6A24A]/15 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-[#b57910]"><ShieldCheck className="h-4 w-4" /> Trust &amp; authenticity</span><h1 className="mt-6 font-serif text-4xl font-extrabold text-gray-900 sm:text-5xl lg:text-6xl">Certificates you can verify</h1><p className="mx-auto mt-5 max-w-3xl text-base leading-7 text-gray-700 sm:text-lg">Our credentials reflect independent recognition, responsible standards and the transparent quality practices behind OrganoCity.</p></div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-14 sm:px-6 sm:py-18">
        ...
        {featured ? <article className="mt-12 grid overflow-hidden rounded-2xl border border-[#C6A24A]/20 bg-white text-gray-900 shadow-lg md:grid-cols-2">
          ...
          <div className="flex flex-col justify-center p-7 sm:p-10"><span className="flex w-fit items-center gap-2 rounded-full bg-[#ffedd5] px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-[#ea580c]"><Award className="h-4 w-4" /> Featured certificate</span><Organization certificate={featured} /><h2 className="mt-6 font-serif text-3xl font-extrabold text-gray-900">{featured.title}</h2><p className="mt-4 leading-7 text-gray-600">{featured.shortDescription || "A verified OrganoCity credential supporting our commitment to quality and trust."}</p><CertificateMeta certificate={featured} />{featured.verificationUrl ? <a href={featured.verificationUrl} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex w-fit items-center gap-2 rounded-lg bg-[#f6a45d] px-5 py-3 text-sm font-bold text-white hover:bg-[#d8861f]">Verify certificate <ExternalLink className="h-4 w-4" /></a> : null}</div>
        </article> : null}
      ...
      <section className="bg-[#fcf5e8] px-5 py-16 text-center"><SearchCheck className="mx-auto h-10 w-10 text-[#f6a45d]" /><h2 className="mt-4 font-serif text-3xl font-extrabold text-gray-900">Need to verify a credential?</h2><p className="mx-auto mt-3 max-w-2xl text-gray-600">Open a certificate above, use its verification link, or contact our team for supporting information.</p><div className="mt-7 flex flex-wrap justify-center gap-3"><Link href="/contact" className="rounded-lg bg-[#f6a45d] px-6 py-3 text-sm font-bold text-white hover:bg-[#d8861f]">Contact OrganoCity</Link><Link href="/products" className="rounded-lg border border-[#C6A24A]/40 bg-white px-6 py-3 text-sm font-bold text-gray-900">Explore our products</Link></div></section>
